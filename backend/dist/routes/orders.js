...
router.get('/cart', auth_js_1.authMiddleware, async (req, res) => {
    try {
        const result = await db_js_1.pool.query(`SELECT c.id, c.product_id, c.quantity, p.title as product_title, p.price as product_price, p.image_url as product_image
       FROM cart_items c
...
router.post('/cart', auth_js_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const productCheck = await db_js_1.pool.query('SELECT * FROM products WHERE id = $1', [product_id]);
        if (productCheck.rows.length === 0) {
            return res.status(404).json({ message: 'Product not found' });
        }
        if (quantity <= 0) {
            // Delete if quantity is set to 0 or negative
            await db_js_1.pool.query('DELETE FROM cart_items WHERE user_id = $1 AND product_id = $2', [req.user.id, product_id]);
            return res.json({ message: 'Item removed from cart' });
        }
        // Upsert query
        await db_js_1.pool.query(`INSERT INTO cart_items (user_id, product_id, quantity)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id, product_id)
       DO UPDATE SET quantity = $3`, [req.user.id, product_id, quantity]);
        // Fetch updated cart items list
        const updatedCart = await db_js_1.pool.query(`SELECT c.id, c.product_id, c.quantity, p.title as product_title, p.price as product_price, p.image_url as product_image
       FROM cart_items c
       JOIN products p ON c.product_id = p.id
       WHERE c.user_id = $1`, [req.user.id]);
...
router.delete('/cart/:productId', auth_js_1.authMiddleware, async (req, res) => {
    const { productId } = req.params;
    try {
        await db_js_1.pool.query('DELETE FROM cart_items WHERE user_id = $1 AND product_id = $2', [req.user.id, productId]);
...
router.post('/', auth_js_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        // 1. Create the order
        const orderResult = await client.query(`INSERT INTO orders (user_id, total_amount, status, shipping_name, shipping_address, shipping_city, shipping_postal)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [req.user.id, totalAmount, 'pending', shipping_name, shipping_address, shipping_city, shipping_postal]);
        ...
        for (const item of orderItemsToInsert) {
         ...
         VALUES ($1, $2, $3, $4)`, [newOrder.id, item.product_id, item.quantity, item.price]);
...
router.get('/history', auth_js_1.authMiddleware, async (req, res) => {
    try {
        const ordersResult = await db_js_1.pool.query('SELECT * FROM orders WHERE user_id = $1 ORDER BY id DESC', [req.user.id]);
        const orders = ordersResult.rows;
        // Fetch items for each order
        for (const order of orders) {
            const itemsResult = await db_js_1.pool.query(`SELECT o.id, o.product_id, o.quantity, o.price, p.title as product_title, p.image_url as product_image
         FROM order_items o
