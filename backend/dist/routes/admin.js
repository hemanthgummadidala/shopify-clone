...
router.get('/stats', auth_js_1.adminMiddleware, async (req, res) => {
    try {
        // 1. Total Sales & Total Orders
        const salesResult = await db_js_1.pool.query(`
      SELECT 
    ...
    `);
        // 2. Total Products
        const productsResult = await db_js_1.pool.query('SELECT COUNT(id) as total_products FROM products');
        // 3. Total Users
        const usersResult = await db_js_1.pool.query('SELECT COUNT(id) as total_users FROM users');
        // 4. Sales by Status
        const statusResult = await db_js_1.pool.query(`
      SELECT status, COUNT(id) as count, COALESCE(SUM(total_amount), 0) as amount
      FROM orders
      GROUP BY status
    `);
        // 5. Recent Orders
        const recentOrdersResult = await db_js_1.pool.query(`
...
router.get('/users', auth_js_1.adminMiddleware, async (req, res) => {
    try {
...
router.delete('/users/:id', auth_js_1.adminMiddleware, async (req, res) => {
    const { id } = req.params;
    try {
        // Check if user exists
        const userResult = await db_js_1.pool.query('SELECT role FROM users WHERE id = $1', [id]);
        ...
        await db_js_1.pool.query('DELETE FROM users WHERE id = $1', [id]);
